...
function AppInner() {
  ...
  const [kontaktDialog, setKontaktDialog] = useState({ open: false, initialType: "Probetraining" });

  const isMobile = useMediaQuery("(max-width: 1200px)");
  const pages = useMemo(() => ["Startseite", "Über uns", "Training", "Ausbildung", "Kontakt"], []);
  ...
  const openKontaktDialog = (initialType = "Allgemein") => {
    setKontaktDialog({ open: true, initialType });
    setMobileOpen(false);
  };
  ...
  const renderContent = () => {
    switch (activePage) {
      case "Über uns":
        return <About />;
      case "Training":
        return <Training onNavigate={handleNavClick} onOpenContactDialog={openKontaktDialog} />;
      case "Ausbildung":
        return <Ausbildung />;
      case "Kontakt":
        return <Kontakt onNavigate={handleNavClick} onOpenContactDialog={openKontaktDialog} />;
      case "Impressum":
        return <Impressum />;
      case "Datenschutz":
        return <Datenschutz />;
  ...
  return (
    <ThemeProvider theme={theme}>
          ...
          <Toolbar sx={{ py: 1, justifyContent: "space-between", color: "text.primary" }}>
            ...
            {!isMobile ? (
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Divider orientation="vertical" flexItem sx={{ mx: 0.8, borderColor: alpha("#0B1B24", 0.1) }} />
                  ...
                  {legalPages.map((item) => (
                    <Button
                      ...
                      sx={{
                        ...
                        fontWeight: 700,
                        "&:hover": { background: alpha(theme.palette.primary.main, 0.06), color: "text.primary" },
                      }}
                    >
                      {item}
                    </Button>
            ...
            <List sx={{ p: 0 }}>
              {pages.map((item) => (
                <ListItem key={item} disablePadding>
                  ...
                  >
                    <ListItemText primary={item} primaryTypographyProps={{ sx: { fontWeight: item === activePage ? 800 : 650 } }} />
                  </ListItemButton>
                </ListItem>
              ))}
                  ...
                  >
                    <ListItemText
                      ...
                      primaryTypographyProps={{
                        sx: { fontWeight: item === activePage ? 800 : 700, color: alpha(theme.palette.text.primary, 0.78) },
                      }}
                    />
                  </ListItemButton>
                </ListItem>
      ...
      <KontaktDialog
        open={kontaktDialog.open}
        initialType={kontaktDialog.initialType}
        onClose={() => setKontaktDialog((prev) => ({ ...prev, open: false }))}
